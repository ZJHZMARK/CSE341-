...
class Point < GeometryValue
  ...
  def intersectWithSegmentAsLineResult(seg)
    if inbetween(@x, seg.x2, seg.x1) and inbetween(@y, seg.y2, seg.y1)
      self
    else
      NoPoints.new
    end
...
class Line < GeometryValue
  ...
  def intersectLine(line)
    if real_close(@m, line.m) then
      ...
      temp = (line.b - @b) / (@m - line.m)
      Point.new(temp, @m * temp + @b)
    end
  end
...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    if real_close(@x1, @x2) and real_close(@y1, @y2)
      Point.new(@x1, @y1)
    elsif (real_close(@x1, @x2) and (@y2 > @y1)) || (@x2 > @x1)
  ...
  def helper1(aXend, aYend, aXstart, aYstart,
  ...
  def helper2(aXend, aYend, aXstart, aYstart,
              bXend, bYend, bXstart, bYstart)
  ...
  def intersectWithSegmentAsLineResult(seg)
    if real_close(@x1, @x2) then
      if @y2 < seg.y2 then
        helper1(@x1, @y1, @x2, @y2, seg.x1, seg.y1, seg.x2, seg.y2)
      else
        helper1(seg.x1, seg.y1, seg.x2, seg.y2, @x1, @y1, @x2, @y2)
      end
    else
      if @x2 < seg.x2 then
        helper2(@x1, @y1, @x2, @y2, seg.x1, seg.y1, seg.x2, seg.y2)
      else
