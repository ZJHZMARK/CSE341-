...
val test2a = assoc("foo", [("ok", 1)]) = NONE				     
val test2b = assoc ("a", [("a", 1), ("b", 2), ("c", 3)]) = SOME(1)				       			 
val test3 = case dot (json_obj, "ok") of SOME True => true |  _ => false
val test3a = dot(json_obj, "ok")
val test4 = one_fields json_obj = rev ["foo","bar","ok"]
val test4a = not(one_fields json_obj = rev ["ok", "bar", "foo"])
val test4b =not( one_fields json_obj = []	)			       
val test5 = not (no_repeats ["foo","bar","foo"])
val test5a = no_repeats["foo", "bar", "ok"]
...
val nest2 = Array [Object [],
                  Object[("a",True),
                         ("b",Object[("foo",True),
                                     ("foo",True)]),
                         ("c",True)]]
val test6a = not (recursive_no_field_repeats nest2)
...
val test7b = count_occurrences (["b", "a", "b"], Fail "") = []
             handle (Fail "") => true
val test7c = count_occurrences(["a", "a", "b", "b", "b"], Fail "") = [] handle (Fail "") => true
											   
					    
val test8 = string_values_for_field ("x", [Object [("a", True),("x", String "foo")],
                                           Object [("x", String "bar"), ("b", True)]])
            = ["foo","bar"]
val test8a = string_values_for_field("x", [Object[("a", True), ("x", String "foo")], Object[("x", String "bar")],
					   Object[("x", String "ok")]]) = ["foo", "bar", "ok"]
...
val test16a = concat_with("a", ["p", "p", "b", "b"]) = "papabab"						   

val test17 = quote_string "foo" = "\"foo\""
val test17a = not (quote_string "foo" = "foo")			       
